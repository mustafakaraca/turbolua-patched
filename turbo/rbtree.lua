...
local function tree_search (x, k, Tnil, compare)
   while x ~= Tnil do
      local res = compare(k, x.key)
      if res == 0 then
         return x
      end
      if res < 0 then
...
local function tree_search_ge (x, k, Tnil, compare)
   local last
   while x ~= Tnil do
      local res = compare(k, x.key)
      if res == 0 then
...
local function tree_search_le (x, k, Tnil, compare)
   local last
   while x ~= Tnil do
      local res = compare(k, x.key)
      if res == 0 then
...
local function rb_insert (T, z, compare)
   local y = T.sentinel
   local x = T.root
   while x ~= T.sentinel do
      y = x
      if compare(z.key, x.key) < 0 then
         x = x.left
      else
         x = x.right
      end
   ...
   if y == T.sentinel then
      T.root = z
   elseif compare(z.key, y.key) < 0 then
      y.left = z
   else
      y.right = z
   end
...
function _M.new (compare)
   local sentinel = rbtree_node()
   sentinel.color = BLACK
   
   return setmetatable({ root = sentinel, sentinel = sentinel, compare = compare or default_compare }, mt)
end
...
function _M.insert (self, key)
   key = rbtree_node(key)
   rb_insert(self, key, self.compare)
end


function _M.find (self, key)
   local z = tree_search (self.root, key, self.sentinel, self.compare)
...
function _M.findge (self, key)
   local z = tree_search_ge (self.root, key, self.sentinel, self.compare)
   if z == self.sentinel then
      return nil
   end
   return z.key
...
function _M.findle (self, key)
   local z = tree_search_le (self.root, key, self.sentinel, self.compare)
   if z == self.sentinel then
      return nil
   end
   return z.key
