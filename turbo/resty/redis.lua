...
function _M.new(host, port, timeout)
    local success, res, err = pcall(turbo.iosimple.dial, string.format('tcp://%s:%d', host or "127.0.0.1", port or 6379), nil, nil, timeout)
    if not success then
        return nil, 'TCP dial failed in pcall with: ' .. tostring(res)
    end
    if not res then
        return nil, 'TCP dial failed with: ' .. tostring(err)
    end
    return setmetatable({ _sock = res, _subscribed = false, _timeout = timeout }, mt)
end
...
local function _line_striped(line, start)
    local n = #line
    while true do
        local b = byte(line, n)
    ...
    return sub(line, start, n)
end

local function _read_reply(self, sock)
    local line, err = sock:read_until("\n", self._timeout)
    if not line then
    ...
    if prefix == 36 then    -- char '$'
        ...
        local data, err = sock:read_bytes(size, self._timeout)
        if not data then
            if err == "timeout" then
                sock:close()
            end
        ...
        local dummy, err = sock:read_bytes(2, self._timeout) -- ignore CRLF
        if not dummy then
            return nil, err
        end
    ...
    elseif prefix == 45 then    -- char '-'
        ...
        return false, _line_striped(line, 2)
...
local function _do_cmd(self, ...)
    ...
    sock:write(table.concat(req), self._timeout)
...
function _M.commit_pipeline(self)
    ...
    sock:write(table.concat(reqs), self._timeout)
